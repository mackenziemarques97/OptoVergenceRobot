classdef ExperimentClass_GUI_LEDboard < handle %define handle class
    ...
    methods %define methods of the handle class (functions to use)
        ...
        function obj = ExperimentClass_GUI_LEDboard(COM) 
            ...
            %open connection
            %startTalk = tic;
            obj.connection = serialport(COM,115200,"DataBits",8,"Timeout",35); 
        ...
        function sendRobotPhaseParams(obj,color,x1,y1,duration,currentPhase,startPhase,lastPhase,currentTrial,startTrial,lastTrial)
            str1 = sprintf('sendRobotPhaseParams:%s:',color);
            str2 = sprintf('%d:%d:%d:%d:%d:%d:',[x1,y1,duration,currentPhase,startPhase,lastPhase,currentTrial,startTrial,lastTrial]);
            sendInfo = [str1 str2]       
            writeline(obj.connection, sendInfo); 
            waitSignal = check(obj);
        end
        ...
        %% findDimensions
        % move robot LED to xMax from current location then to xMin and counts and stores the number of steps it took
        % does the same in the z-direction
        % Returns the number of steps in a 2-element array, x & z dimension
        % Ends at (xMin, zMin)/the origin
        function findDimensions(obj)            
            %send string in this format with colon delimiter to Arduino
            writeline(obj.connection,'findDimensions:');       
            waitSignal = check(obj);
        end
